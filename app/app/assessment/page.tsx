...
import type { Category, SkillItem, RoleTarget, User, Role, Level } from "@/types"
...
function AssessmentContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const found = (users as User[]).find((u) => u.id === userId)
      ...
      for (const item of master.skillItems) {
        initial[item.id] = { currentLevel: 0, evidence: "" }
      }
      setAnswers(initial)
      setLoading(false)
    })
