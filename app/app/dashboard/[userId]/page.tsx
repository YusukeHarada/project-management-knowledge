...
import type { User, Assessment, Category, SkillItem, RoleTarget, GapAnalysis, CategoryScore, Role } from "@/types"
...
export default function PersonalDashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([users, assessments, master]: [User[], Assessment[], { categories: Category[]; skillItems: SkillItem[]; roleTargets: RoleTarget[] }]) => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 全項目一覧 */}
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <h2 className="text-lg font-semibold mb-4">全項目ギャップ一覧</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-gray-500 text-xs">
                ...
                <th className="text-center py-2 pr-3 w-20">現状</th>
            ...
            <tbody>
              {gapAnalyses.map((g) => (
                <tr key={g.skillItem.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-2 pr-3 text-center font-medium">Lv{g.currentLevel}</td>
